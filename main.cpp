...
void Get_time(BST a,char** Q)
{
    ...
    for (i=0;i<N;i++)
    {
        a.GetSer(Q[i]);//searching each word in the BST class
    }
    output<<"Time taken for the creation of BST: "<<(double)(clock() - tStart)/CLOCKS_PER_SEC<<endl;

    if (C==1)
    {
        for (i=0;i<N;i++)
        {
            a.printsearch(Q[i], output);
        }
    }
    output.close();
}
